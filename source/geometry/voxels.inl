	...
	triangle const* ignore_triangle, int ignore_material) const
{
	...
	if(z >= _size_z)
	{
		...
		delta_S.x = std::ceil((((z - _size_z) / -dz) - delta_S.x)) / std::abs(dx) + delta_S.x; // calculate distances to the first intersections with x and y planes 
		delta_S.y = std::ceil((((z - _size_z) / -dz) - delta_S.y)) / std::abs(dy) + delta_S.y; //   such that z > 0
